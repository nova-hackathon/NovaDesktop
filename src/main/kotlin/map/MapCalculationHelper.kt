...
class MapCalculationHelper(private val distances: Map<String, DistanceInfo>) {
    ...
    private val unknownPositions: ArrayBlockingQueue<DistanceInfo> = ArrayBlockingQueue(distances.keys.size)
    private val knownPositions = ArrayList<DevicePosition>()
    private var currentDevice: DistanceInfo = DistanceInfo("unknown", LinkedHashMap(), false,0,0)
    ...
    fun calculate(): java.util.ArrayList<DevicePosition> {
        ...
        val secondStartDevice = distances[startDevice.phoneName]?.distanceMap?.keys?.first() ?: ""
        ...
        knownPositions.add(DevicePosition(distances[secondStartDevice]!!, secondStartDevicePosition))
    ...
    private fun calculateDevicePosition(
        ...
        val absPosition = Pair(positionCoordinates.first.absoluteValue, positionCoordinates.second.absoluteValue)
